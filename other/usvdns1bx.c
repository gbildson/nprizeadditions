...
void removeUV()
{
	...
	for(u=0;u<NUSERS;u++) {
		...
		int i;
		double sUu=sU[u];
		double ssUu=sigmoid(sUu);
		for(i=0; i<d012;i++) {
			err[base0+i]-=ssUu*sV[ent[i]&USER_MOVIEMASK];
...
static double dsignorm = (M_E/10.);

double dsigmoid(double x) {
	double expv   = exp(x);
	double denom = (1. + expv);
	denom = denom * denom;
	double result = expv / denom;
	result /= dsignorm;
...
int uucount=0;
			for(u=0;u<NUSERS;u++) {
				...
				for(j=0;j<d0;j++) {
					...
					double sVm=sV[m];
					double t = (*e++) - ssUu*sVm; // New estimation error
					rmse+=t*t;
					vslope[m]+=t*(sUu);
					uslope+=t*(sVm);
if ( (uucount++ % 8000000) == 0 ) {
