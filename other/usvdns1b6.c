...
int score_train(int loop)
{
	...
	/* Initial estimation for current feature */
	{
		int m;
		for(m=0;m<NMOVIES;m++) {
			sV[m]=0.1*drand48() - 0.05; 
			sW[m]=0.1*drand48(); 
			//sW[m]=0.1*drand48() - 0.05; 
			//sV[m]=0.1*((double)rand()/(double(RAND_MAX)+1.0)) - 0.05; 
			//sW[m]=0.1*((double)rand()/(double(RAND_MAX)+1.0)) - 0.05; 
		}
	}
...
//}
			...
			double tsV;
			for(m=0;m<NMOVIES;m++) {
				tsV = sV[m] + LRATE*vslope[m] - LRATE_LAMBDA*sV[m];
				//if ( tsV < 0.0 ) 
					//tsV = 0.0;
				sV[m] = tsV;
