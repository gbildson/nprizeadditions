...
//foobar(int argc, char**argv) {
	...
	if(fname_qualify) {
		...
		char *qualify_path="data/probe.bin";
		unsigned int *qualify=malloc(QQPP*4);
		load_bin(qualify_path,qualify,QQPP*4);
		unsigned int *q=qualify;
		while (q<(qualify+QQPP)) {
			int m=*q++;
			fprintf(fp,"%d:\n",m+1);
			int l=*q++;
			int j;
			for(j=0;j<l;j++) {
				...
				int base2=useridx[u][0]+useridx[u][1];
				int d2=+useridx[u][2];
				//printf("m: %d l: %d base: %d d2: %d u: %d\n", m, l, base2, d2, u);
				int k;
				for(k=0;k<d2;k++) {
				...
				int r=(userent[base2+k]>>USER_LMOVIEMASK)&7;
				r++;
				float e=err[base2+k];
		        float p=r-e; // extract prediction from real results and error
		        //float p=r; // extract prediction from real results and error
				fprintf(fp,"%.1f\n",p);
			}
		}
		fclose(fp);
