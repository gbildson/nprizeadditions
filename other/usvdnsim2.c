...
//int uucount=0;
			for(u=0;u<NUSERS;u++) {
				...
				for(j=0;j<d0;j++) {
					...
					double t = (*e++) - facts; // New estimation error
					...
					printf("M: %.8d\tsV: %.6f\tE: %.6f\tsUxsV: %.6f\trmse: %.6f\tuS: %.6f\tvS: %.6f\n", m, sVm, t, facts, rmse, vslope[m], uslope);
...
printf("RMSE = %0.5f\n", rmse);
			...
			int m;
			for(m=0;m<NMOVIES;m++) {
				double sVdelta = LRATE*vslope[m] - LRATE_LAMBDA*sV[m];
				double sWdelta = LRATE*wslope[m] - LRATE_LAMBDA*sW[m];
				sV[m] += LRATE*vslope[m] - LRATE_LAMBDA*sV[m];
				sW[m] += LRATE*wslope[m] - LRATE_LAMBDA*sW[m];
				printf("M: %d\tsVd: %.6f\tsWd: %.6f\tsV: %.6f\tsW: %.6f\n", m, sVdelta, sWdelta, sV[m], sW[m]);
			}
