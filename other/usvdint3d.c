...
#define UUG0 (0.0111) // for biases
#define UUL4 (0.0450) // for biases

void setupBUJForWFactors() {
	...
	while( ( (prmse<=last_prmse) || loopcount < 6) ) {
		...
		for(u=0;u<NUSERS;u++) {
			...
			for(j=0;j<d0;j++) {
				...
				float wbUu = wbU[u];
				float wbVm = wbV[m];
				wbU[u] += Gamma0 * (e2 - wbUu * UUL4);
				wbV[m] += Gamma0 * (e2 - wbVm * UUL4);
			}
		}
