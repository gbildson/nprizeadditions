...
void ZERO3D(double ***Arr3D) {
    memset(Arr3D[0][0],0, (TOTAL_FEATURES * 5 * NMOVIES * sizeof(double)));  
}

void allocate3D(double **** narr) {
    double *space;
	double ***Arr3D;
    int y, z;

    space = (double *) malloc(TOTAL_FEATURES * 5 * NMOVIES * sizeof(double));
    Arr3D = (double ***) malloc(NMOVIES * sizeof(double **));
	*narr = Arr3D;

    for (z = 0; z < NMOVIES; z++) {
        Arr3D[z] = (double **) malloc(5 * sizeof(double *));
        for (y = 0; y < 5; y++) {
            //Arr3D[z][y] = (double *) space + (z*(NMOVIES * 5) + y*TOTAL_FEATURES);
            Arr3D[z][y] = (double *) space + (z * 5) * TOTAL_FEATURES + y * TOTAL_FEATURES; 
        }
    }
}
