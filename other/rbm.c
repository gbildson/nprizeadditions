...
unsigned int vishidCNT[NMOVIES][5][TOTAL_FEATURES];
unsigned int visbiasesCNT[TOTAL_INPUTS];
unsigned int hidbiasesCNT[TOTAL_FEATURES];
...
//);
			...
			for(h=0;h<TOTAL_FEATURES;h++) {
    	        hidbiases[h] += EpsilonHB /*/ ((double)(d0*5))*/ * ((poshidact[h] - neghidact[h]) + weightcost * hidbiases[h]);
//printf("h: %d, cnt: %d\n", h, hidbiasesCNT[h]);
//fflush(stdout);
//printf("hb: %f, pa: %f, na: %f, d0:%d\n", hidbiases[h], poshidact[h], neghidact[h], d0);
			...
			for(j=0;j<d0;j++) {
				...
				int rr;
				for(rr=0;rr<5;rr++) {
					if ( posvisact[(m*5+rr)] == 0.0 && negvisact[(m*5+rr)] == 0.0 ) continue;
					visbiases[(m*5+rr)] += EpsilonVB * ((posvisact[(m*5+rr)] - negvisact[(m*5+rr)]) + weightcost * visbiases[(m*5+rr)]);
//printf("m: %d, s: %d, cnt: %d\n", m, rr, visbiasesCNT[m*5+rr]);
//fflush(stdout);
