...
printf("RMSE GOAL: %f\n", rmse_targ);
	...
	/* Optimize current feature */
	{
		...
		for(last_rmse=1.e20,epoch=0; epoch<MAX_EPOCHS; epoch++) {
			...
			if ( epoch < 5 ) {
				lrate_lambda = LRATE_LAMBDA_ORIG*3.0;
				lrate = LRATE_ORIG*3.0;
			} else {
				lrate_lambda = LRATE_LAMBDA_ORIG;
				lrate = LRATE_ORIG;
			}
...
//int uucount=0;
			for(u=0;u<NUSERS;u++) {
				...
				for(j=0;j<d0;j++) {
					...
					int svcnt = sVCnt[m];
					//double vSlopeFact;
					//double vratio = ((double)svcnt)/vcavg;
					//if (vratio >= 0.4 )
						//vSlopeFact = 2.2*log(vratio)+1.0;
					//else
					    //vSlopeFact = 2.2*vratio+0.15;
					//ssVm = sVm / vSlopeFact;
					if ( svcnt > vcavg*100 ) 
					    ssVm = sVm / 7.0;
...
//}
			...
			if( epoch >= 22 && (deltar < ((1.0/penalty)*0.0000001) || rmse <= rmse_targ) ) break;
